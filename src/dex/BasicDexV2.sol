...
contract BasicDexV2 is ERC20 {
    ...
    constructor(
        ...
        ERC20(
            string.concat(_creditToken.symbol(), _assetToken.symbol(), "LP"),
            string.concat(_creditToken.symbol(), _assetToken.symbol(), "LP"),
            18
        )
    {
    ...
    function init(uint256 tokens) public returns (uint256) {
        if (totalSupply != 0) revert InitError();
    ...
    function withdraw(
        ...
        if (balanceOf[msg.sender] < amount)
            revert InsufficientLiquidityError(balanceOf[msg.sender]);

        uint256 creditTokenReserve = creditToken.balanceOf(address(this));
        uint256 assetTokenReserve = assetToken.balanceOf(address(this));

        creditTokenAmount = (amount * creditTokenReserve) / totalSupply;
        assetTokenAmount = (amount * assetTokenReserve) / totalSupply;
